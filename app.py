...
def gerar_pdf_visual(df_ranking):
    ...
    data = [df_ranking.columns.to_list()] + df_ranking.values.tolist()
    ...
    # Colorir linhas conforme nível de risco
    for i, row in enumerate(df_ranking["Nível de risco"], start=1):
        if "Alto" in row:
            style.add('BACKGROUND', (0,i), (-1,i), colors.HexColor("#fff0f0"))
        elif "Médio" in row:
            style.add('BACKGROUND', (0,i), (-1,i), colors.HexColor("#fffbea"))
        else:
...
# Uso no Streamlit
pdf_buffer = gerar_pdf_visual(ranking)
st.download_button(
    label="📥 Baixar score (PDF)",
    data=pdf_buffer,
    file_name="ranking_absenteismo.pdf",
